...
(async()=>{
  ...
  try {
    ...
    await drag([465,490],[435,265]);assert.equal((await audio()).events.correct,1);
    ...
    await drag([635,490],[200,300]);assert.deepEqual((await audio()).events,events,'mute suppresses all effects');
